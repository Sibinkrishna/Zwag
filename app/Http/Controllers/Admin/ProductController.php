...
class ProductController extends Controller
{
    ...
    public function create()
    {
        ...
        $brands = Brand::all();
        // dd($categories);
        return view('admin.product.create', compact('subtitle', 'metatitle', 'categories', 'attributes','brands'));
    }
            ...
            DB::transaction(function () use ($request) {
                ...
                foreach ($request->price as $i => $price) {
                    ...
                    ProductVariant::create([
                        ...
                        'tag' => isset($request->tag[$i])
                                ? (is_array($request->tag[$i])
                                    ? implode(',', $request->tag[$i])
                                    : $request->tag[$i])
                                : null,
                        'product_associate' => json_encode($request->product_associate[$i] ?? []),
                    ]);
                }
