...
// side menu separate active function

document.addEventListener("DOMContentLoaded", function () {

    function activateParentForSubChild() {
        const currentPage = window.location.pathname.split("/").pop();

        // Find all hidden sub-child links
        const childLinks = document.querySelectorAll(".sub-child-link");

        childLinks.forEach(child => {
            const childHref = child.getAttribute("href");
            if (childHref !== currentPage) return;

            // Get closest nav-item
            const navItem = child.closest(".nav-item");
            if (!navItem) return;

            // Highlight the main visible nav-link
            const mainLink = navItem.querySelector(".nav-link:not(.sub-child-link)");
            ...
            const collapseDiv = child.closest(".collapse");
            if (collapseDiv) {
                // Expand collapse
                collapseDiv.classList.add("show");

                // Highlight the parent menu arrow
                const navArrow = document.querySelector(`a[href="#${collapseDiv.id}"]`);
                if (navArrow) navArrow.classList.add("active");

                // If part of sub-nav-item, highlight only main sub-nav-link
                const subNavItem = child.closest(".sub-nav-item");
                if (subNavItem) {
                    const subMainLink = subNavItem.querySelector(".sub-nav-link:not(.sub-child-link)");
                    if (subMainLink) subMainLink.classList.add("active");
                }
            }
        });
    }

    activateParentForSubChild();
});
